...
class LLMProvider(Enum):
    ...
    CLAUDE = "claude"
    QWEN = "qwen"
    ALIBABA = "alibaba"  # Alias for Qwen


def get_active_provider() -> LLMProvider:
    """Determine active provider from environment variables."""
    # Check for Qwen/Alibaba first
...
class QueryEnginePort:
    ...
    provider: LLMProvider = field(default_factory=get_active_provider)

    @classmethod
    def from_workspace(cls, provider: Optional[LLMProvider] = None) -> 'QueryEnginePort':
        return cls(
    ...
    def submit_message(
        ...
        if self.provider == LLMProvider.QWEN:
            output = self._call_qwen_api(prompt)
        else:
            # Default Claude-style mock response
            summary_lines = [
                f'Prompt: {prompt}',
    ...
    def _call_qwen_api(self, prompt: str) -> str:
        ...
        try:
            from .qwen_client import QwenClient, QwenRequest, QwenMessage
            ...
            return "[Qwen client not available]"
