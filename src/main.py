...
from .query_engine import QueryEnginePort, LLMProvider
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bootstrap_parser.add_argument('--provider', choices=['claude', 'qwen', 'alibaba'], help='LLM provider to use')
    ...
    loop_parser.add_argument('--provider', choices=['claude', 'qwen', 'alibaba'], help='LLM provider to use')
    ...
    flush_parser.add_argument('--provider', choices=['claude', 'qwen', 'alibaba'], help='LLM provider to use')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'summary':
        ...
        print(f"**Active Provider: {engine.provider.value}**\n")
        print(summary)
        return 0
    ...
    if args.command == 'flush-transcript':
        provider = LLMProvider(args.provider) if args.provider else None
        engine = QueryEnginePort.from_workspace(provider=provider)
